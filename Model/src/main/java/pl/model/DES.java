...
public class DES {
    ...
    /// plan jest taki jak na wykladzie zeby pierw uzupelnic te bity ze do adresow 1-4,7-10,13-17 itd i zostawic miejsce miedzy nimi zeby te skopiowane tam wjebac
    public byte[] expansion(byte[] msg){
        BigInteger help = new BigInteger(msg);

        byte[] afterExpansion = new byte[6];
        BigInteger expanded = new BigInteger(afterExpansion);
        for (int i = 0; i < 32;i++) {
            if(help.testBit(i)) {
                expanded.setBit(i + i / 4);
                System.out.print(i + i / 4);
                System.out.print(" ");
            }
        }

        for (byte c : expanded.toByteArray()) {
